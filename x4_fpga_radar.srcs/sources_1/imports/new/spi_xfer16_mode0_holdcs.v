// ============================================================
// spi_xfer16_mode0_holdcs.v
// 16-bit SPI transfer core, Mode0 (CPOL=0, CPHA=0), MSB-first
// - Sample MISO on rising edge
// - Shift MOSI on falling edge
// - hold_cs=1 keeps CS low between back-to-back transfers
// F_sck = F_clk / (2*CLK_DIV)
// ============================================================
module spi_xfer16_mode0_holdcs #(
    ...
    input  wire        hold_cs,    // NEW: keep CS low between transfers
        ...
        if (!rst_n) begin
            ...
            case (state)
                ST_IDLE: begin
                    ...
                    // 如果 hold_cs=0，则空闲时释放 CS；如果 hold_cs=1，则保持当前 cs_n（可保持低）
                    if (!hold_cs)
                        cs_n <= 1'b1;
                    ...
                    if (div_cnt == CLK_DIV-1) begin
                        ...
                        if (sck == 1'b0) begin
                            ...
                            if (bit_cnt == 5'd0) begin
                                rx16 <= sh_rx;   // 已包含最后一次 rising edge 采样
                                sck  <= 1'b0;
                                busy <= 1'b0;
                                done <= 1'b1;
